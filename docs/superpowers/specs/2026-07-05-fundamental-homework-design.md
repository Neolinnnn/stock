...
python scripts/fundamental_homework.py --auto --top 5  # 讀當日 summary.json 行動清單前 N 檔
python scripts/fundamental_homework.py 2330 --force    # 忽略 7 天快取重做
```

- 7 天內已產出的功課直接沿用（比對 `docs/fundamental/{sid}.json` 的 `generated_at`），控制 Gemini 額度。
- `--auto` 讀 `daily_reports/{今日}/summary.json` 的行動清單個股，逐檔執行，已有新鮮快取者跳過。
...
- `docs/fundamental/{sid}.json`：`{stock_id, name, generated_at, revenue, financials, valuation, chip, checklist, score, narrative}`
- `docs/fundamental/index.json`：已完成功課清單（sid、name、score、generated_at）
